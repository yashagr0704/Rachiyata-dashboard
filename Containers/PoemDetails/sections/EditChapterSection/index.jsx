...
const EditChapterSection = ({ poemId, data, isSuccess: isFetchSuccess, isLoading: isFetchLoading }) => {
  const [showMessageOnSave, setShowMessageOnSave] = useState(false)
  const { enqueueSnackbar } = useSnackbar()
  const { handleFormError } = useFormError()
  const { handleSubmitChapter, isLoading, isSuccess } = useCreateOrUpdatePoemChapterAPI(
    poemId,
    undefined,
    data?.id,
    showMessageOnSave,
  ...
  }, [poemId, data?.author_note, data?.chapter_content, data?.chapter_sequence, data?.chapter_title, data?.id, methods])
    ...
    <Root>
      <FormProvider {...methods}>
        <Row>
          ...
          <Link href={`/workspace/poem/${poemId}`}>
            <StyledLink>
