...
import { useCreateOrUpdatePoemChapterAPI, useDeleteChapterAPI } from 'Containers/PoemDetails/api/poem.hook'

const ChapterBar = ({ refetch, poemId, item }) => {
  ...
  const { handleSubmitChapter, isLoading: isUpdateLoading } = useCreateOrUpdatePoemChapterAPI(
    item?.poem_id,
  ...
  return (
    <Root>
      ...
      <StyledButton variant="contained" startIcon={<ModeEditOutlineOutlinedIcon />}>
        <StyledEditA
          target="_blank"
          href={`/workspace/poem/edit-chapter?poemId=${poemId}&&chapterId=${item?.id}`}
          rel="noreferrer"
        />
        Edit
      </StyledButton>
