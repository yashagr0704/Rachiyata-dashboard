...
import { useCreateOrUpdatePoemAPI, useDeletePoemAPI } from 'Containers/PoemDetails/api/poem.hook'
...
const InfoArea = ({ item }) => {
  ...
  const { handleDeletePoem, isLoading: isDeleteLoading, isSuccess: isDeleteSuccess } = useDeletePoemAPI(item?.id)
  const {
    handleSubmitPoem,
    isLoading: isUpdateLoading,
    isSuccess: isUpdateSuccess,
  } = useCreateOrUpdatePoemAPI(true, item?.id)
  ...
  return (
    <Root>
      <DeleteModal
        messageNotice={`Do you want to Delete Poem Named ${item?.poem_name} ?`}
      ...
      />
      <InfoModal
        messageNotice={`Do you want to Publish Poem Named ${item?.poem_name} ?`}
        open={isPublishModalOpen}
        setOpen={setIsPublishModalOpen}
        isLoading={isUpdateLoading}
        onClickOk={() => {
          handleSubmitPoem({ ...item, poemId: item?.id, is_published: true })
        }}
      />
      <InfoModal
        messageNotice={`Do you want to Unpublish Poem Named ${item?.poem_name} ?`}
        open={isUnpublishModalOpen}
        setOpen={setIsUnpublishModalOpen}
        isLoading={isUpdateLoading}
        onClickOk={() => {
          handleSubmitPoem({ ...item, poemId: item?.id, is_published: false })
        }}
      />
      {/* <InfoModal
        messageNotice={`Do you want to Restore Poem Named ${item?.poem_name} ?`}
      ...
      /> */}
      <PoemName variant="h3">{item?.poem_name}</PoemName>
      <InfoChipList>
      ...
      <ButtonList>
        <Link href={`/workspace/poem/edit/${item?.id}`}>
