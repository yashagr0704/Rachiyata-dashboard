...
import {
  useCategoryDropdown,
  useCheckBookName,
  useCreateOrUpdateNovelAPI,
} from 'Containers/NovelDetails/api/novel.hook'
...
const CreateOrEditNovelSection = ({ editMode, data, isFetchSuccess, isLoading: isFetchLoading }) => {
  const { dropdown: categoryDropdown } = useCategoryDropdown()
  const { handleSubmitNovel, isLoading } = useCreateOrUpdateNovelAPI(editMode, data?.id)
  const { handleFormError } = useFormError()

  const methods = useForm({
    defaultValues: {
      bookId: editMode ? data?.id : '',
      book_name: editMode ? data?.book_name : '',
  ...
  const { isTaken: isBookNameTaken, message: messageBookName } = useCheckBookName(
    methods.watch('book_name'),
    data?.book_name,
  )
  const { isTaken: isAlternateNameTaken, message: messageAlternateName } = useCheckBookName(
    ...
    <Root>
      <Main>
        ...
        <FormProvider {...methods}>
          ...
          <Body>
            <Left>
              <StyledTextField
                enableColors
                isCorrect={!isBookNameTaken}
                helperText={messageBookName}
                placeholder="Novel name here..."
                name="book_name"
                label="Name"
              />
              <StyledTextField
        ...
        <SaveButton
          ...
          variant="contained"
          onClick={methods.handleSubmit(handleSubmitNovel, handleFormError)}
          color="primary">
          Save
        </SaveButton>
      </Main>
...
export default CreateOrEditNovelSection
